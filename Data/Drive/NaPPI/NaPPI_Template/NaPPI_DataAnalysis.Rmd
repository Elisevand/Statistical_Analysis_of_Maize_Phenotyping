...
violin2 <- ggplot(endpoint_clean, aes(y = FW_shoot_g, x = Genotype))+
  geom_violin(aes(fill = Plant_type), color = "white", alpha = 0.4)+
  geom_sina(size=1, aes(color = Plant_type))+
  theme(legend.position = "none")+
...
variables_1 <- colnames(df_S_timeseries[, c(5:(genotype_index - 1))])
variables_1
```
...
plot(timePoint_endpoint,
     ...
     highlight =  c("EPPN20_T", "EPPN06_H", "EPPN08_H", "EPPN10_L", "EPPN05_H", "EPPN11_H", "EPPN09_H", "EPPN04_H", "EPPN03_H", "EPPN12_H", "EPPN10_H", "EPPN01_H", "EPPN02_H", "EPPN11_L","EPPN13_H", "EPPN07_L"))
```
...
```{r}
variables_voulues <- c(variables_1[1], variables_1[3], variables_1[5], variables_1[7], variables_1[8])
traits <- variables_voulues
...
nn <- 0.6 # nearest neighbour (0 - 1 greater number smoother curve)
ce <- FALSE
```
...
for (trait_name in traits) {
  ...
  if (exists(mod_name)) {
    ...
    # Tracer le graphique pour les valeurs brutes prédites avec lignes
    plot(mod,
         plotType = "rawPred",
         plotLine = TRUE)
    
    # Tracer le graphique pour les valeurs de corrélation prédites avec lignes
    plot(mod,
         plotType = "corrPred",
         plotLine = TRUE)
    
    # Tracer le graphique pour les valeurs d'héritabilité avec une limite Y spécifiée
    plot(mod, 
         plotType = "herit",
         yLim = c(0, 0.5))
    
    # Tracer le graphique pour les dimensions effectives avec un type de ED et une limite Y spécifiés
    plot(mod,
         plotType = "effDim",
         EDType = "ratio",
         yLim = c(0, 1))
...
```{r}
## remove all point/plant with NA in the S_Height_cm 
colNum <- which(names(Spatial_Corrected_Out) %in% paste0(trait_name, "_corr"))
Spatial_Corrected_Out <- Spatial_Corrected_Out[!is.na(Spatial_Corrected_Out[,colNum]),]
Spatial_Corrected_Out <- droplevels(Spatial_Corrected_Out)
...
param1bis <- estimateSplineParameters(x = fit.splineBIS,
                           estimate = "derivatives",
                           what = "max")
head(param1bis)

param1bis[,'genotype'] <- factor( param1bis[, 'genotype'], 
                              levels = c("EPPN20_T", "EPPN06_H", "EPPN08_H", "EPPN10_L", "EPPN05_H", "EPPN11_H", "EPPN09_H", "EPPN04_H", "EPPN03_H", "EPPN12_H", "EPPN10_H", "EPPN01_H", "EPPN02_H", "EPPN11_L", "EPPN13_H", "EPPN07_L"))
