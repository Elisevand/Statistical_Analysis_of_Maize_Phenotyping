...
setwd("~/Mémoire/Main/Data/Drive/ALSIA")
library(statgenHTP)
library(tidyr)
library(dplyr)
library(gridExtra)
...
data <- read.delim("raw_destructive.txt")
data_imaging_raw <- read.delim("raw_imaging.txt")
data_imaging <- read.delim("extracted_imaging.txt")
data_water <- read.delim("raw_water.txt")

data_imaging$timestamp <- as.Date(data_imaging$timestamp)
# EXPLORATION PLOTS

# Scatter plot pour les time series : pour height_above_reference.S, projected_shoot_area, wue

ggplot(data_imaging, aes(x = timestamp, y = height_above_reference.S))+
  geom_point(aes(colour = genotype))+
  geom_smooth(aes(colour = genotype))+
  labs(title = "Evolution de la height_above_reference")+
  facet_wrap(~genotype)

ggplot(data_imaging, aes(x = timestamp, y = projected_shoot_area))+
  geom_point(aes(colour = genotype))+
  geom_smooth(aes(colour = genotype))+
  labs(title = "Projected shoot area evolution over time by genotype")+
  facet_wrap(~genotype)

ggplot(data_imaging, aes(x = timestamp, y = wue))+
  geom_point(aes(colour = genotype))+
  geom_smooth(aes(colour = genotype))+
  labs(title = "Evolution de la wue")+
  facet_wrap(~genotype)
