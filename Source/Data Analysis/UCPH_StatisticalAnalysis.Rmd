---
title: "UCPH_StatisticalAnalysis"
author: "Elise"
date: "`r Sys.Date()`"
output: 
  html_document:
...
knitr::opts_knit$set(root.dir = "C:/Users/elise/Documents/Mémoire/Main/Data/Templates/UCPH")
```
...
```{r}
check_factor_levels <- function(data, factors) {
  for (factor in factors) {
    num_levels <- nlevels(as.factor(data[[factor]]))
    if (num_levels < 2) {
      stop(paste("Factor", factor, "must have at least two levels but has", num_levels))
    } else {
      print(paste("Factor", factor, "has", num_levels, "levels"))
    }
  }
}

# Example of factors to check in your dataset
factors_to_check <- c("Genotype", "Row", "Column", "Soil")
check_factor_levels(endpoint_clean, factors_to_check)
